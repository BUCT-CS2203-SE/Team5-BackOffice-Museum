...
import dash_callback.application.message_.post_comment_c  # noqa: F401
...
def render_content(menu_access: MenuAccess, **kwargs):
    return [
        fac.Fragment(
            [
                fuc.FefferyTimeout(id='comment-init-timeout', delay=1),
            ]
        ),
        fac.AntdSpace(
            [
                fac.AntdSpace(
                    [
                        fac.AntdSelect(
                            ...
                            mode='multiple',
                            placeholder=t__notification('请选择评论状态'),
                            style={'width': '200px'},
                        ),
                        fac.AntdPopconfirm(
                            fac.AntdButton(
